...
#########################################################################
#
# $Header: /mnt/raid/services/cvs/python/packages/share1.5/Vision/VPE.py,v 1.238.2.1 2015/08/26 18:53:47 sanner Exp $
#
# $Id: VPE.py,v 1.238.2.1 2015/08/26 18:53:47 sanner Exp $
#
...
from NetworkEditor.items import NetworkNode, ImageNode
...
from NetworkEditor.items import NodeStyle

class NodeStylesManager:

    def __init__(self):
        self.styles = {}

    def getStylesForNode(self, node):
        if node.library is None:
            return None
        key = '%s__%s'%(node.library.name, node.__class__.__name__)
        return self.styles.get(key, None)
        
    def addStyle(self, node, name, styleObject):
        key = '%s__%s'%(node.library.name, node.__class__.__name__)
        if not self.styles.has_key(key):
            self.styles[key] = {'default':name}
        assert isinstance(styleObject, NodeStyle)
        self.styles[key][name] = styleObject
        
    def setDefault(self, node, name):
        key = '%s__%s'%(node.library.name, node.__class__.__name__)
        self.styles[key]['default'] = name

    ## def loadStylesFromFile(self, filename):
    ##     stylesDict = getObjectFromFile(os.path.abspath(filename), 'nodesStyles')
    ##     for k, v in stylesDict.items():
    ##         d = {}
    ##         #get the default style name
    ##         d['default'] = v.pop('default')
    ##         for name, style in v.items():
    ##             d[name] = NodeStyle(**style)
    ##             #print 'MAKING STYLES', k, name, style
    ##         self.styles[k] = d
        
    def loadStylesFromFolder(self, folder):
        from glob import glob
        for styleFile in glob(os.path.join(folder, "*__*.py")):
            styles = getObjectFromFile(styleFile, 'styles')
            k = os.path.split(styleFile)[1][:-3].replace('___', '.')
            d = {}
            d['default'] = styles.pop('default', None)
...
"""
    ...
    def getNodeLib(self, libName, package, version=None):
        # libName is the name of the NodeLibrary object in package_version.py
        # Returns a library and a message. Library can be None and msg can be ""
        # try to get it from loaded libraries
        #print 'LOOKING FOR', package, libName, version
        lib = self.libraries.get(libName, None)

        if lib: 
            # a library with this name is already loaded
            if version==lib.version or version in lib.compatibleWith:
                #print 'getNodeLib FOUND', lib, lib.name, lib.version
                return lib, ""
            else:
                msg = "Version mismatch, requested version %s but version %s already loaded"%(version, lib.version)
                return None, msg
        else: # we load the library and return the handle
            module = package
            if version:
                module += "_%s"%(version.replace(".", "_"))
            #print module, libName
            mod = __import__(module, globals(), locals(),[libName])
            for modItemName in dir(mod):
                modItem = getattr(mod, modItemName)
                #print modItem
                if isinstance(modItem, NodeLibrary):
                    self.addLibraryInstance(modItem, module, libName)
                    if modItem._hasImageNodes:
                        self.addToTree(package, modItem)
                    #print 'LOADED', modItem, modItem.name, modItem.version
                    return modItem, ""
        return None, "No library found for %s %s"%(package, libName, version)
    ...
    def addLibraryInstance(self, lib, modName, varName, force=False):
        #print 'FUGU1', lib, modName, varName, force
...
class VisualProgramingEnvironment(NetworkBuilder, NoGuiVPE):
    ...
    def addToTree(self, package, nodelib):
        alreadyThere = False
        for child in self.libTree.rootNode.children:
            if child.name==package:
                alreadyThere = True
                break
        if not alreadyThere:
            return self.libTree.addLibrary(nodelib)
        
            
    def deleteFromTree(self, package):
        alreadyThere = False
        for child in self.libTree.rootNode.children:
            if child.name.split()[0]==package:
                item = self.libTree.tree.objToItem[child]
                self.libTree.tree.item_delete(item)
                self.libTree.rootNode.children.remove(child)
                del self.libTree.tree.objToItem[child]
    ...
    def __init__(self, name='NetworkBuilder', master=None, font=None,
        ...
        # Create a main PanedWidget with menu bar, module library, button bars,
        # and editor going from top to bottom
        ...
        # create a pane for menu bar
        menus = self.top.add('MenuBars', min=20, max=40, size=20)

        # create a pane for libraries notebook
        ...
        # create a pane for the standard button bar
        self.buttonBar = self.top.add('ButtonBar', min=20, max=20, size=20)
        self.buttonBarFunc = {} # key:buttonName, value:function

        # add more button bars (potentially)
        self.addMoreButtonBars()

        # create a pane for libtree and networks notebook
        edtab = self.top.add('Editor', min=100)

        # inside the edtab create a pane widget with a left pane for libtree
        # and a right pane for networks
        pane = self.treeNetpane = Pmw.PanedWidget(
            edtab, orient='horizontal', handlesize=-1,
            separatorthickness=10, separatorrelief='raised', )
        
        self.libtreeMaster = pane.add('libtree', min=10, size=200)
        ed = right = pane.add('networks', min=10, size=400)
        handle = pane.component('handle-1')
        sep = pane.component('separator-1')
        pane.component('handle-1').place_forget()
        pane.component('handle-1').forget()
        pane.component('handle-1').pack_forget()
        pane.component('handle-1').grid_forget()
        pane.component('separator-1').configure(
            bd =2, highlightthickness=1, highlightbackground='black',
            highlightcolor='black')

        # separator for rearranging 
        spacer = Tkinter.Frame(right, width=10)
        spacer.pack(expand=0, fill='y',side='left', anchor='w')

        w = Tkinter.Frame(sep, height=40, width=4, bg='#fffeee',
                           relief='sunken', bd=1, highlightbackground='black',
                           highlightthickness=1)
        w.pack(anchor='center', padx=2, pady=2, side='left', expand=0,
                fill=None)
...
##             self.registerListener(DeleteConnectionsEvent, self.printEvent)
        ...
        nodeStylesDir = ensureVisionResourceFile()
        self.nodeStylesManager = NodeStylesManager()
        self.nodeStylesManager.loadStylesFromFolder(nodeStylesDir)
        ...
        self.root.protocol("WM_DELETE_WINDOW", self.interactiveExit_cb)

        self.hideLibraries()
        from Vision.libTree import LibTree
        self.libTree = LibTree(self, self.libtreeMaster)
        self.treeNetpane.configurepane('libtree', size=200)
...
##                                            lib.varName)
    ...
    def resize_cb(self, event=None):
        ...
        # before libtree we used event.width
        #width = event.width
        width = self.top.winfo_width()
        #print 'WIDH_AAA', event.width, self.top.winfo_width()
        if self.width != width:
            curwidth = self.width
            self.width = width
...
you need to install: %s \n""" %(module,dep))
        ...
        for nodelib in modItems:
            if nodelib._hasImageNodes:
                self.addToTree(module, nodelib)
...
class NodeLibrary:
    ...
    def __init__(self, name, color='gray75', mode='readOnly',
                 fillColor=None, outlineColor=None, version='',
                 compatibleWith=[]):
        self.name = name
        ...
        self.nodeNames = []
        self.nodeClassFromName = {}
        self._hasImageNodes = False # set to true when ImageNode instance is
                                    # added. Needs to be True for library to
                                    # show in libtree widget
...
"""
    ...
    def addNode(self, nodeClass, name, category, args=None, kw=None,
        ...
        if issubclass(nodeClass, ImageNode):
            self._hasImageNodes = True
...
#            warnings.warn(txt)
        ...
        self.nodeClassFromName[nodeClass.__name__] = nodeClass
...
onto the network editor canvas."""
    ...
    def resizeCategories(self):
        ...
        # return if the libraries are not shown
        menu = self.ed.menuButtons['Libraries'].menu
        try:
            menu.index("Show libraries")
            shown = False
        except:
...
"""
        ...
        if issubclass(node.nodeClass, ImageNode):
            n1.buildSmallIcon(sc_canvas, 10, posy, font)
        elif hasattr(vpe,'drawPortInLibraryGui') and \
               vpe.drawPortInLibraryGui is False:
            n1.buildSmallIcon(sc_canvas, 10, posy, font)
        else:
