...
Input Ports
    inArray: numpy array or list of data
...
"""
    ...
    def __init__(self, name='AsType', **kw):
        ...
        self.dtypes= [
            'character', 'complex', 'complex128', 'complex192',
            'complex64',  'float', 'float16', 'float32',
            'float64', 'float96', 'int', 'int0', 'int16', 'int32', 'int8',
            'uint', 'uint0' ,'uint16', 'uint32', 'uint8', 'uint64', 'unsignedinteger'
            ]
    ...
    if isinstance(data, numpy.ndarray):
        self.outputData(outArray=data.astype(getattr(numpy,dtype)))
    else:
        d = numpy.array(data)
        self.outputData(outArray=d.astype(getattr(numpy,dtype)))
"""
...
"""
    ...
    if operator=='normalize':
        norm = 1./numpy.sqrt(numpy.add.reduce(data*data, 1))
        norm.shape = norm.shape+(1,)
        result = data * norm
    else:
        op = getattr(numpy, operator)
        result = op(numpy.array(data))
...
"""
    ...
    if operator=='': return
    op = getattr(numpy, operator)
    result = op(numpy.array(inArrayList1), numpy.array(inArrayList2) )
    self.outputData(outArrayList=result)
"""
...
Output Ports:
  ...
  """
    ...
    if isInstance(valueList) is True:
        ...
        elif isinstance(valueList, numpy.ndarray):
            outputClass = numpy.array
...
class ReadField(NetworkNode):
    """Reads a field (list, tuple or numpy array) using pickle.Pickler
Double-clicking on the node opens a text entry widget to type the file name.
In addition, double-clicking in the text entry opens a file browser window.
...
class SaveField(NetworkNode):
    """Saves a field (list, tuple or numpy array) using pickle.Pickler
Double-clicking on the node opens a text entry widget to type the file name.
In addition, double-clicking in the text entry opens a file browser window.
...
"""
    ...
    ed = self.getEditor()
    if type(color) in [types.ListType, numpy.ndarray, types.TupleType]:
        assert len(color) == 3 or len(color) == 4
        color = list(color)
        if numpy.minimum.reduce(color) <= 1:
            color = numpy.multiply(color, 255)
...
handles lists (of ints, floats, arrays, strings) or numpy arrays.
...
#         self.constrkw['functionOrString'] != functionOrString ):
    ...
    if function is not self.function:
        ...
        #from ViewerFramework.VFCmds import VFCmd
        #if isinstance(function, VFCmd):
        #    args = getargspec(function.doit.im_func)
        #el
...
stdlib.addNode(EntryFieldNE, 'EntryField', 'Input')
stdlib.addNode(EntryFieldIntNE, 'EntryInt', 'Input')
stdlib.addNode(EntryFieldFloatNE, 'EntryFloat', 'Input')
stdlib.addNode(EntryFieldTimeNE, 'EntryTime', 'Input')
stdlib.addNode(EntryFieldDateNE, 'EntryDate', 'Input')
stdlib.addNode(EntryFieldHexNE, 'EntryHex', 'Input')
