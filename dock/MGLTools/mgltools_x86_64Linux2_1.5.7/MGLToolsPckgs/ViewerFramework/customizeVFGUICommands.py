...
#
# $Header: /mnt/raid/services/cvs/python/packages/share1.5/ViewerFramework/customizeVFGUICommands.py,v 1.33 2011/09/30 23:25:34 annao Exp $
#
# $Id: customizeVFGUICommands.py,v 1.33 2011/09/30 23:25:34 annao Exp $
#
...
class BindActionToMouse(Command):
    ...
    def negateCmdBefore(self, action, buttonNum, modifier='None',
                        actionDict='Object'):
        ...
        if value[0]:
            ...
            #                 {}, self.name)
            self.undoCmds = ([(self, (action, value[0], value[1], actionDict), {})],
                             self.name)
        ...
        #                 {}, self.name)
        return ([(self, (oldaction, buttonNum, modifier, actionDict),
                  {})], self.name)
...
class ChangeFont(Command):
    ...
    def negateCmdBefore(self, newfont):
        font = self.getCurrentFont()
        #self.addUndoCall( (font,), {}, self.name )
        return ([(self, (font,), {})], self.name )
...
class showHideGUI(Command):
    ...
    def negateCmdBefore(self, name, onOff):
        #
        #self.addUndoCall( (name, not onOff), {}, self.name )
        return ([( self, (name, not onOff), {})], self.name )
