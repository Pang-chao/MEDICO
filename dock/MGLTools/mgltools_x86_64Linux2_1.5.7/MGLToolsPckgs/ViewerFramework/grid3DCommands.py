...
#########################################################################
# 
# $Header: /mnt/raid/services/cvs/python/packages/share1.5/ViewerFramework/grid3DCommands.py,v 1.21.4.2 2016/02/25 01:20:17 annao Exp $
# $Id: grid3DCommands.py,v 1.21.4.2 2016/02/25 01:20:17 annao Exp $
   ...
   \ndoit(self, grid3D, name=None):\n
    ...
    def doit(self, grid3D, name=None):        
        ...
        if not hasattr(grid3D,'geomContainer'):
            ...
            self.vf.GUI.VIEWER.AddObject(g)
        ...
        for gridSettings in settings:
            ...
            if gridSettings.has_key('isoBarNumber'):
                ...
                if data.dtype.char!=numpy.float32:
                    data = data.astype('f')
                self.vf.Grid3DIsocontour.newgrid3D = numpy.reshape( 
            ...
            if gridSettings.has_key('LUTintervals_list'):
                ...
                mapper = MapGridData()
                result = mapper(grid.data, datatype=numpy.uint8, datamap=datamap,
                                powerOf2=True)
                gtype = ArrayTypeToGrid[result.dtype]
                if grid.crystal:
    ...
    def plot_signature(self):
        ...
        for s in range(4):
            if hasattr(self.cmd.grid,'signature'):
                ...
                sig =self.cmd.grid.signature[s]               
            x = numpy.zeros( (sig.nval,), 'f')
            sig.getFx(x)
            y = numpy.zeros( (sig.nval,), 'f')
            sig.getFy(y)
            max_y = max(y)
            min_y = min(y)
   ...
   \nOptional Arguments:\n  
    ...
    def doit(self, grid3D, material = None, isovalue = None , 
        ...
        if name in  grid3D.geomContainer['IsoSurf']:
            ...
            self.vf.GUI.VIEWER.AddObject(g, parent = grid3D.IsoSurf)
...
class myLUT(LUT):
    ...
    def __init__(self, master=None, xminval=0,xmaxval=255, ymaxval=4095,
       ...
       #                    fill='', tags='outline')
        ...
        self.color_arr = numpy.zeros((arr_len,3),'f')
        self.alpha_arr = numpy.zeros(arr_len).astype(numpy.int16)
        self.points = []
        self.values = []
        self.shapes = []
        self.isoVals = []
...
#        self.menu.add_separator()
    ...
    def colorGUI(self, tag=None):
        ...
        if hasattr(self,'ColorMapGUI'):
            ...
            ramp = RGBARamp()
            ramp[:,3] = numpy.arange(0,0.25,1./(4*256.),'f')
            name += "-VolRender Colormap"
    ...
    def ColorMap_cb(self, colorMap):
        ramp = numpy.array(colorMap.ramp)
    ...
    def setMax(self, text):
        ...
        datamap['map_type'] = 'linear'
        mapper = MapGridData()
        result = mapper(grid.data.copy(), datatype=numpy.uint8, datamap=datamap,
                        powerOf2=True)
    ...
    def merge_function(self):
        ...
        for lut in self.lut_list[1:]:
            ...
            alpha_arr = numpy.concatenate((alpha_arr, lut.alpha_arr))
            color_arr = numpy.concatenate((color_arr, lut.color_arr))
...
##              print 'in load_file: interval =', interval
            ...
            lut.color_arr = numpy.reshape(numpy.array(colors[i], 'f'),
                                            colors_size)
...
class VolRenCommand(Command):
    ...
    def select(self):
        ...
        if not hasattr(grid,'volRenGrid'):
            ...
            self.grid = grid
            alphas = numpy.take(grid.LUT_data.alpha_arr,values)
...
commandList = [
    ...
    {'name':'Grid3DCommands','cmd':Grid3DCommands(),'gui':Grid3DGUI},    
