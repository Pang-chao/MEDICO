...
#########################################################################
#
# $Header: /mnt/raid/services/cvs/python/packages/share1.5/NetworkEditor/simpleNE.py,v 1.241.2.1 2015/08/26 18:52:12 sanner Exp $
#
# $Id: simpleNE.py,v 1.241.2.1 2015/08/26 18:52:12 sanner Exp $
#
...
"""
    ...
    def __init__(self, name='NoName', master=None, font=None, withShell=1, **kw):
        ...
        # used as default to create new connections
        self.createBlockingConnections = True
    ...
    def resize_cb(self, event=None):
        if self.visibleWidth != event.width:
            ...
            for net in self.networks.values():
                net.naviMap.placeVisibleWin()
                

    def resizeNetworkArea(self):
        # compute how much width the individual panes get
        if self.winfo_width() == 1: # first time of instantiation
...
##                                      command=self.resetWidgetValues_cb)
    ...
    def createMacro(self, network, name, klass=None):
        if klass is None:
...
#            self.deleteNetwork(self.currentNetwork)
    ...
    def refreshNet_cb(self, net=None, event=None):
        ...
        naviMap = net.naviMap
        for node in net.nodes:
            node.drawMapIcon(naviMap)
        for conn in net.connections:
            conn.drawMapIcon(naviMap)
...
"""
    ...
    def sourceFile(self, resourceFile=None):
        ...
        if not path:
            return
        self._sourceFile(path)


    def _sourceFile(self, filepath):
        ...
        execfile( filepath, globalDict, localDict)
