...
#########################################################################
#
# $Header: /mnt/raid/services/cvs/python/packages/share1.5/NetworkEditor/itemBase.py,v 1.112.2.1 2015/08/26 18:52:11 sanner Exp $
#
# $Id: itemBase.py,v 1.112.2.1 2015/08/26 18:52:11 sanner Exp $
#
    ...
    def deleteIcon(self):
        ...
        if self.iconMaster:
            ...
            if self.menu:
                self.menu.unpost()
                self.network.postedMenu = None
## MS when the code below is enabled, after deleteing a connection the mouse
## button event are missing the Shift key in event.
##                 self.menu.destroy()
                self.menu = None
    ...
    def buildIcons(self, canvas):
        ...
        if issubclass(self.__class__, ImageNode):
            master = ed.libTree.paramPanelMaster
        else:
            master = None

        self.paramPanel = ParamPanel(self, master=master, title=self.name,
                                     applyFun=self.schedule_cb,
                                     dismissFun=self.hideParamPanel)
        self.paramPanel.mainFrame.forget()
