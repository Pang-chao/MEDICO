...
#########################################################################
#
# $Header: /mnt/raid/services/cvs/python/packages/share1.5/NetworkEditor/ports.py,v 1.209.2.2 2016/02/11 23:41:06 annao Exp $
#
# $Id: ports.py,v 1.209.2.2 2016/02/11 23:41:06 annao Exp $
#
...
"""
    ...
    ## def getSize(self, portNum):
    ##     return self[portNum].get('size', 20)

    ## def getLabel(self, portNum):
    ##     return self[portNum].get('label', None)

    ## def getEdge(self, portNum):
    ##     descr = self[portNum]
    ##     found = False
    ...
    ##         print 'PORT EDGE NOT FOUND %rpos key missing', portNum, descr
    ##         return 'top'
    ##     if k=='ulrpos':
    ...
    ## def getDrawingParams(self, portNum, node):
    ##     # get the parameters from drawing this port
    ##     # the position can be specified usinf ulrpos=(x,y) to specify an offset
    ##     # from the upper left corner of the node's box. Likewise urrpos,
    ##     # llrpos and lrrpos can be used. The offset can be integer for pixels
    ##     # of a float < 1.0 for a percentage of the node's width.
    ##     # the 'size' is return (default 20)
    ##     # the 'fill' is the color used for the port's background
    ##     # the 'line' is the color used for the port's arrow
    ##     # the 'outline' is the color used for the port's outline
    ##     # vector is a vector normal the node's edge, pointing outside in
    ##     #        coord system where +Y is up
    ##     ulx, uly = node.UL
    ...
    ##     size = self.getSize(portNum)
    ##     fill = descr.get('fill', (1,1,1,1))
    ##     line = descr.get('line', (0,0,0,1))
    ##     outline = descr.get('outline', (0,0,0,1))
    ##     label = self.getLabel(portNum)

    ##     dx, dy = descr.get('ulrpos', (None, None))
    ##     if dx is not None:
    ##         if abs(dx) < 1.0: dx *= width
    ##         if abs(dy) < 1.0: dy *= height
    ##         #print 'UL', ulx, uly, width, height, dx, dy, descr.get('ulrpos'), ulx+dx, uly+dy
    ##         return ulx+dx, uly+dy, size, vector, line, fill, outline, label

    ##     dx, dy = descr.get('urrpos', (None, None))
    ##     if dx is not None:
    ##         if abs(dx) < 1.0: dx *= width
    ##         if abs(dy) < 1.0: dy *= height
    ##         #print 'UR', ulx, uly, width, height, dx, dy, descr.get('urrpos'), ulx+width+dx, uly+dy
    ##         return ulx+width+dx, uly+dy, size, vector, line, fill, outline, label

    ##     dx, dy = descr.get('llrpos', (None, None))
    ##     if dx is not None:
    ##         if abs(dx) < 1.0: dx *= width
    ##         if abs(dy) < 1.0: dy *= height
    ##         #print 'LL', ulx, height, dx, dy, descr.get('llrpos'), ulx+dx, uly+height+dy
    ##         return ulx+dx, uly+height+dy, size, vector, line, fill, outline, label

    ##     dx, dy = descr.get('lrrpos',(None, None))
    ##     if dx is not None:
    ##         if abs(dx) < 1.0: dx *= width
    ##         if abs(dy) < 1.0: dy *= height
    ##         #print 'LR', ulx, width, dx, descr.get('lrrpos'), ulx+width+dx, uly+height+dy
    ##         return ulx+width+dx, uly+height+dy, size, vector, line, fill, outline, label

    ##     # fixme .. find a good location for this port
    ##     return ulx+10, uly, size, vector, line, fill, outline, label
...
class InputPortsDescr(PortsDescr):
    ...
    def append(self, *args, **kw):
        ...
        if not kw.has_key('inputPortClass'):
            #from NetworkEditor.ports import ImageInputPort
            from NetworkEditor.items import ImageNode
            if isinstance(self.node(), ImageNode):
                kw['inputPortClass'] = ImageInputPort
        PortsDescr.append(self, *args, **kw)
...
class OutputPortsDescr(PortsDescr):
    ...
    def append(self, *args, **kw):
        ...
        if not kw.has_key('inputPortClass'):
            #from NetworkEditor.ports import ImageOutputPort
            from NetworkEditor.items import ImageNode
            if isinstance(self.node(), ImageNode):
                kw['outputPortClass'] = ImageOutputPort
        PortsDescr.append(self, *args, **kw)
...
class Port(NetworkItems):
    ...
    def __init__(self, name, node, datatype='None',
                 ...
                 shape=None, color=None, originalDatatype=None, vector=None
                 ):
        ...
        self.height = height
        self.shape = shape
        self.vector = vector      # vector orthogonal to shape edge, pointing outside
        self.vectorRotated = vector # vector orthogonal to shape edge, pointing outside
        self.color = color
...
"""
    ...
    def destinationPortsCoords(self, tag):
        ...
        # also look for ImageNodes
        from NetworkEditor.items import ImageNode
        for node in self.node.network.nodes:
            if node == self.node: continue
            if not isinstance(node, ImageNode): continue
            x, y = node.posx, node.posy
            for p in node.inputPorts:
                c.append( (p.posRotated[0]+x, p.posRotated[1]+y) )
                il.append( p )
        
        return numpy.array(c, 'i'), il
...
"""
    ...
    def drawRubberBand(self, event=None):
        ...
        dist = numpy.sum( dist*dist, 1 )
        indsort = numpy.argsort( dist )
        portId = self.ipId[indsort[0]]
        #portId can be a canvas item for regular nodes or an InputPort
        #for ImageNodes
        if isinstance(portId, Port):
            port = portId
        else:
    ...
    def connectNodesEnd(self, event=None):
        ...
        self.createConnection( self.secondNodePort, **kw)


    def createConnection(self, port2, **kw):
        ed = self.node.network
        port1 = self
        if isinstance(self, SpecialOutputPort):
            conn = ed.specialConnectNodes(
                *(port1.node, port2.node, port1.number, port2.number), **kw )
        else:
            conn = ed.connectNodes(
                *(port1.node, port2.node, port1.number, port2.number), **kw )
...
"""
        ...
        if isinstance(self, (OutputPort, ImageOutputPort)):
            origDescr = dummy.outputPortsDescr[id]
            #print "origDescr", origDescr
        elif isinstance(self, (InputPort, ImageInputPort)):
        ...
        for k,v in ownDescr.items():
            if k in origDescr.keys():
                try:
                    ...
                    if (numpy.array(v) - numpy.array(origDescr[k])).any():
                       descr[k] = v 
                ownDescr.pop(k)
...
class InputPort(Port):
    ...
    def __init__(self, name, node, datatype=None, required=True,
        ...
        Port.__init__(self, name, node, datatype=datatype, balloon=balloon, 
                      ...
                      originalDatatype=originalDatatype, vector=vector)
        ...
        if isinstance(self.widget, NGWidget) is False:
          ...
          if wasExpanded and len(self.node.widgetDescr.keys()):# repack node
            ...
            if resize is True and isinstance(self.widget, NGWidget) is False: # only resize, if we have no node widget left
                if hasattr(self.node, 'nodeWidgetMaster'):
...
"""
        ...
        #Check is this input port's data has already been validates
            ...
            if isinstance(typeObj, AnyType):
                ...
                # check that data can be turned into numpy array
                #import pdb;pdb.set_trace()
                ...
                # lArray is now a numpy array of the right data type
...
class OutputPort(Port):
    ...
    def __init__(self, name, node, datatype='None', balloon=None,
                 ...
                 beforeConnect=None, afterConnect=None, beforeDisconnect=None,
                 afterDisconnect=None, shape=None, color=None, vector=(0,-1)):
        ...
        Port.__init__(self, name, node, datatype, balloon, width, height,
                      ...
                      afterDisconnect, shape=shape, color=color, vector=vector)
...
class RunNodeInputPort(SpecialInputPort):
    ...
    def __init__(self, node, datatype='triggerIn', vector=(-1,0)):

        SpecialInputPort.__init__(self, 'runNode', node, datatype,
                                  balloon='trigger this node', vector=vector)
        
        

class RunChildrenInputPort(SpecialInputPort):

    def __init__(self, node, datatype=None, vector=(-1,0)):

        SpecialInputPort.__init__(
            self, 'runChildren', node, datatype,
            balloon="trigger this node's children", vector=vector)
...
class TriggerOutputPort(SpecialOutputPort):
    ...
    def __init__(self, node, vector=(1,0)):

        SpecialOutputPort.__init__(self, 'trigger', node, 'triggerOut',
                                   balloon="trigger connected node")

##
## ImagePort are port used by ImageNodes
##
class ImagePort:

    def __init__(self):
        self.pos = [0,0] # in node's coord system (i.e relative to ul)
        self.posRotated = [0,0] # in node's coord system (i.e relative to ul)
        self.mouseAction['<Shift-Button-1>'] = self.startMovePort
        self.posx = 0
        self.posy = 0
        self.id = -1 # Connection.updatePosition returns port.if is None
        self._hasMoved = False
        

    def startMovePort(self, event=None):
        canvas = self.node.iconMaster
        num = event.num
        canvas.bind("<B%d-Motion>"%num, self.movePort)
        canvas.bind("<ButtonRelease-%d>"%num, self.endMovePort)
        self.intersectionPointID = canvas.create_oval( 0, 0, 1, 1, fill='red')
        self.newpos = None
        self.movePort(event)

        
    def movePort(self, event=None):
        p0 = self.node.center
        canvas = self.node.iconMaster
        p1 = canvas.canvasx(event.x), canvas.canvasy(event.y)
        vx, vy = p1[0]-p0[0], p1[1]-p0[1]
        p1 = p0[0]+99999*vx,  p0[1]+99999*vy
        xi, yi = self.node.segmentIntersection(p0, p1)
        #print 'INTERSECTION', xi, yi
        if xi:
            # FIXME I need to move out by size/2 along perpendicular to edge
            canvas.coords(self.intersectionPointID, xi-5, yi-5, xi+5, yi+5)
            self.newpos = (xi,yi)


    def endMovePort(self, event=None):
        canvas = self.node.iconMaster
        canvas.unbind("<B%d-Motion>"%event.num)
        canvas.unbind("<ButtonRelease-%d>"%event.num)
        canvas.delete(self.intersectionPointID)
        del self.intersectionPointID
        if self.newpos:
            nx, ny = self.newpos
            self.moveTo(nx, ny)
            self._hasMoved = True
        del self.newpos
        self.node.currentNodeStyle = None


    def moveTo(self, x, y):
        # x y are in the canvas coordinate system
        self.posx = x
        self.posy = y
        self.node.movePortTo(self, x, y)
    ...
    def getCenterCoords(self):
        hpx, hpy = self.posRotated
        if hasattr(self.node, 'shadowOffset'):
            sx, sy = self.node.shadowOffset
        else:
            sx = sy = 0
        return hpx+self.node.posx+sx, hpy+self.node.posy+sy


            
class ImageInputPort(ImagePort, InputPort):

    def __init__(self, name, node, datatype='None', singleConnection=True,
                 **kw):

        kw1 = {}
        if kw.get('required', None): kw1['required'] = kw['required']
        if kw.get('balloon', None): kw1['balloon'] = kw['balloon']
        if kw.get('width', None): kw1['width'] = kw['width']
        if kw.get('height,', None): kw1['height,'] = kw['height,']
        if kw.get('beforeConnect', None): kw1['beforeConnect'] = kw['beforeConnect']
        if kw.get('afterConnect', None): kw1['afterConnect'] = kw['afterConnect']
        if kw.get('beforeDisconnect', None): kw1['beforeDisconnect'] = kw['beforeDisconnect']
        if kw.get('afterDisconnect', None): kw1['afterDisconnect'] = kw['afterDisconnect']
        if kw.get('shape', None): kw1['shape'] = kw['shape']
        if kw.get('color', None): kw1['color'] = kw['color']
        if kw.get('cast', None): kw1['cast'] = kw['cast']
        if kw.get('originalDatatype', None): kw1['originalDatatype'] = kw['originalDatatype']
        if kw.get('defaultValue', None): kw1['defaultValue'] = kw['defaultValue']
        
        #print 'CREATING PORT', name, kw1
        InputPort.__init__(self, name, node, datatype=datatype,
                           singleConnection=singleConnection, **kw1)
        ImagePort.__init__(self)

        # _hasMoved is used to generate code to move to port when we save
        # the network
        self._hasMoved = False
    ...
    def createWidget(self, rescale=1, descr=None):
        ...
        if descr is None:
            return
        # overwrite master='node' since image nodes do not support widgets
        # in node. We set it to ParamPanel for now
        if self.node.widgetDescr[self.name]['master']=='node':
            self.node.widgetDescr[self.name]['master'] = 'ParamPanel'
        InputPort.createWidget(self, rescale=rescale, descr=descr)

        

   
class ImageOutputPort(ImagePort, OutputPort):

    def __init__(self, name, node, datatype='None', **kw):
        ...
        OutputPort.__init__(self, name, node, datatype, **kw1)
        ImagePort.__init__(self)
