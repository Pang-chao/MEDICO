...
#
# $Header: /mnt/raid/services/cvs/python/packages/share1.5/Pmv/Ribbon.py,v 1.14.8.1 2016/02/11 21:29:43 annao Exp $
#
# $Id: Ribbon.py,v 1.14.8.1 2016/02/11 21:29:43 annao Exp $
#
...
def interpPoints( ctrl, nchord):
    ...
    smooth = numpy.zeros( (((ctrl.shape[0]-2)*nchord+2),4), 'f')
...
def ribdrw( ctrl, nrib, nchord ):
    ...
    tinc = 1./nchord
    smooth = numpy.zeros( (nrib, ((ctrl.shape[0]-2)*nchord+2), 4 ), 'd')
...
##     ctrl = numpy.zeros( (nres, nrib, 4), 'd')
    # MS changes
    ctrl = numpy.zeros( (nres+1, nrib, 4), 'd')
    ...
    for nr in range(nres):
        ...
        if nr<nres-1:
            ...
            if i==0:
                ...
                P = numpy.zeros( (3,), 'd') # No offset for first CA
            else:
              # Not first, ribbon cross vector is average of peptide plane
              # with previous one
              if numpy.dot(D,G)<0.0: B = -D
              else: B = D
              E = G+B
            ...
            P = numpy.zeros( (3,), 'd') # No offset for last CA
...
#    profile.run("smooth, ctrl = ribbon2D( nrib, width, nchords, offset, natoms, cao, [0]*46 );")
    ...
    v = numpy.array(numpy.reshape(smooth, (-1,4))[:,:3])
...
##      v1 = numpy.array(numpy.reshape(smooth1, (-1,4))[:,:3])
