...
#########################################################################
#
# $Header: /mnt/raid/services/cvs/python/packages/share1.5/Pmv/sdCommands.py,v 1.82.2.3 2016/03/03 00:06:41 annao Exp $
#
# $Id: sdCommands.py,v 1.82.2.3 2016/03/03 00:06:41 annao Exp $
#
...
class MolIndexedPolygons(IndexedPolygons):
    ...
    def getTriangles(self, atomindices):
        ...
        displayFlags = -1*numpy.ones(len(self.vertexIndices))
        ...
        for face in self.triangles:
            ...
            if tri0 != -1 and tri1 != -1 and tri2 != -1:
                ...
                thistri.extend([1,0])
                tri.append(thistri)
        return numpy.array(vfloat,'f'), numpy.array(vint,'i'), numpy.array(tri,'i')                     
...
class ReadMolIndexedPolygons(MVCommand):
    ...
    def doit(self, molName, vertfile, trifile, atom_index=None, propertyList=[], name=None, display=1):
        ...
        if display:
            self.vf.displaySurface(mol, name=name, topCommand=0)
...
##                 self.colorMapGUI.update(mini=fmin,maxi=fmax)
            ...
            for g in geometries.values():
                prop = numpy.array(g.displayprops[property]).astype('f')
                #get rid of any other coloring
                colors = self.colorMap.Map(prop)
                colors = numpy.array(colors).astype('f')
...
##                      prop = prop1
##                  prop.shape = (-1,1)
##                  tex = numpy.array(numpy.array(self.colorMap.ramp)*255).astype('B')
##                  t=Texture()
        ...
        if refSetName and mobSetName:
            ...
            if (mobSetName in savedSets.keys() and refSetName
                ...
                if (refSetName != self.refSetName or
                    ...
                    self.mobXYZ = numpy.array(mobXYZ,'f')
                    self.refXYZ = numpy.array(refXYZ,'f')
                ...
                refMatrix = numpy.array(refMatrix,'f')
                refMatrix= numpy.reshape(refMatrix,(4,4))
            ...
            # apply transformation in valid range
                ...
                if refSetName and mobSetName:
                    ...
                    mobMatrix = (master.rotation[:12].tolist() +
                                 master.translation.tolist() + [1.])
                    mobMatrix = numpy.array(mobMatrix,'f')
                    mobMatrix= numpy.reshape(mobMatrix,(4,4))
                    #compute the transformed XYZ
                    refXYZ = numpy.dot(self.refXYZ,refMatrix)
                    mobXYZ = numpy.dot(self.mobXYZ,mobMatrix)
                    #compute the Rmsd
        ...
        def doit(self,mol,fileName,center=None,**kw):
            ...
            if center:
                if center == 'Center on Molecule':
                    ats = mol.allAtoms
                    mol.centerR = numpy.sum(ats.coords, 0)/len(ats.coords)
                    ...
                    Ncoords = 0
                    for set in ats:
                        coordsum = coordsum + numpy.sum(set.coords, 0)
                        Ncoords = Ncoords + len(set.coords)
                    mol.centerR = coordsum/Ncoords
                else:
                    mol.centerR = numpy.array(center)
            ...
            for line in qdata:
                ...
                if len(numbers) < 9:
                    ...
                    self.vf.warningMsg(msg)
                    return None
                trans = numpy.array(map(lambda x: float(x), numbers[1:4]),'f')
                quat = numpy.array(map(lambda x: float(x), numbers[4:8]),'f')
