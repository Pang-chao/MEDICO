...
# $Revision: 1.1.1.24 $ $Date: 2007/01/18 14:43:37 $ kgm

"""SimulationRT 1.8 Provides synchronization of real time and SimPy simulation time.
...
**Change history:**
    ...
    07 Dec 2006: - Changed the __str__ method of Histogram to print a table (like printHistogram)
 
    12 Dec 2006: - Added change to allow setting of simulation to real time ratio during
                   simulation (contributed by Robert C. Ramsdell III -- thanks, Bob!)
"""
__TESTING=False
version=__version__="1.8 $Revision: 1.1.1.24 $ $Date: 2007/01/18 14:43:37 $"
if __TESTING: 
...
class __Evlist:
    ...
    def _nextev(self):
        ...
        if self.real_time and _t < earliest:                
            #~ delay=(1.0*earliest/self.rel_speed)-rtnow()     
            #~ if delay > 0: time.sleep(delay)                 
            next_time = 1.0*(earliest - self.stlast)/self.rel_speed 
            next_time += self.rtlast
            delay=next_time - wallclock()               
            if delay > 0:time.sleep(delay)                  
            self.rtlast = wallclock()                   
            self.stlast = earliest                      
        _t=max(earliest,_t)
