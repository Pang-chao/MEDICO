...
def read_xyz_tensor(atom_tensor, pos_tensor):
    atomic_symbols = []
    xyz_coordinates = []
    atom_dict = ['H', 'C', 'O', 'N', 'F']
    charge = 0
    pos_tensor = pos_tensor.sum(dim=2)

    for i in range(29):
        if atom_tensor[i].sum() == 0:
            break
        for j in range(5):
            if atom_tensor[i][j] == 1.0:
                atomic_symbol = atom_dict[j]
                atomic_symbols.append(atomic_symbol)
                break
        x, y, z = pos_tensor[0][i], pos_tensor[1][i], pos_tensor[2][i]
        xyz_coordinates.append([float(x), float(y), float(z)])
...
def xyztensor2mol(atom_tensor, pos_tensor):
    charged_fragments = True  # alternatively radicals are made
    ...
    # uncomment 'import networkx as nx' at the top of the file
    quick = True
    atomicNumList, charge, xyz_coordinates = read_xyz_tensor(atom_tensor, pos_tensor)
    mol = xyz2mol(atomicNumList, charge, xyz_coordinates, charged_fragments, quick)
...
def tensor2smiles(atom_tensor, pos_tensor, batch_size):
    smiles_list = []
    for i in range(batch_size):
        _, smiles = xyztensor2mol(atom_tensor[i], pos_tensor[i])
...
if __name__ == "__main__":
    ...
    print(tensor2smiles(atom_tensor, pos_xyz_tensor, molecule_num))